...
A personal project that replays the IEEE-CIS fraud dataset as a live payment stream, computes per-card features on the fly, and scores each transaction with a LightGBM model behind a FastAPI endpoint. The main constraint I designed around: offline training and online serving have to compute features the exact same way (train/serve parity). The loop is closed end to end — scheduled retraining, automated validation, gated promotion, and **the running service picking up the new model on its own**.
...
Design decisions worth calling out (see the docstrings in `core/schema.py`, `core/store.py`, `serving/app.py`):
...
| 6 | Validation via column screening, gain importance, SHAP, permutation importance and null importance, then a schema expansion (15 features, **AUC 0.7513 / PR-AUC 0.1837**) |

### What Phase 6 actually did

Every column the schema had discarded up front (415 of them) was pulled back out of the raw CSV and measured for contribution, in stages:
...
Result: `id_31`, `id_19`, `id_20`, `id_29`, `id_30` were added to the schema — **AUC 0.7235 → 0.7513, PR-AUC 0.1140 → 0.1837**.

Null importance also flagged three *existing* features sitting barely above the noise floor: `amt_sum_1h` (0.21), `txn_count_1h` (0.32), `amt_sum_24h` (0.41). They're still in the model, but they are not features whose contribution is established.

### After Phase 6

- **Closed the retraining loop** — mtime-based reloading in `serving/app.py` and atomic file replacement in `offline/promote.py`, so a promoted model reaches the service without a restart. Verified round-trip against a live server (see "How it was verified").
- **Moved Airflow's metadata DB from SQLite to Postgres** — the Postgres container had been declared in `docker-compose.yml` and used by nothing. Zero code changes; three lines in `airflow.cfg`.

## Things that broke, and what fixed them

These are real issues hit during development, kept here instead of glossed over.
...
Root cause: `TCP_NODELAY` wasn't set on the multiprocess socket path, so Nagle's algorithm (hold small packets, batch them) on one side and the client's delayed ACK (hold ACKs, batch them) on the other each waited for the other until the 40ms timer fired. Not "too many workers competing for resources" — a missing socket option.
...
To exercise the gate I deliberately trained a weakened model (`--warmup-days 150`) — and it came back at 0.7443, well *above* the healthy model's 0.7235. The cause: `promote.py` scored candidates on a standard split it built itself, rather than on the split the candidate had actually been trained on. The weak model's training window (day 150–182) sat entirely inside that "held-out" test window (day 140–182). 0.7443 wasn't skill, it was memorization.

Adding a "no train/test overlap" gate caught it exactly: **99,622 of the 118,108 test rows had already been trained on (84%)**. Unlike the statistical gates, this one is binary — overlap or no overlap — so it needs no threshold and carries no risk of rejecting a good model.
...
The first `feature_contribution.py` run threw in every V/C/D/M column as a candidate and pushed AUC from 0.7235 to **0.8806**. But those columns are Vesta's own after-the-fact engineering — a payment terminal cannot send them live. I had violated my own founding design constraint inside the analysis step.
...
### 10. That consolidation missed one caller, and `/score` was dead for three weeks
...
The unsettling part is that **nothing caught it**. All three parity tests kept passing — they only exercise feature *computation* — and the server hadn't been started once since the schema expansion. It surfaced on the very first request when uvicorn came up to verify the model-reload work above.

Two lessons: **grep every caller when consolidating into a shared helper**, and **green unit tests are not evidence that the service starts**.
...
Deliberately left in scope for a portfolio project. Writing them down beats hiding them.
...
Numbers on their own are worth nothing, so here is how each was checked.
