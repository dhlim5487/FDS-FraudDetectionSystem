...
IEEE-CIS 사기 거래 데이터셋을 실시간 결제 스트림처럼 재현하고, 카드별 이력을 기반으로 피처를 계산해 LightGBM 모델로 거래를 스코어링하는 개인 프로젝트입니다. 오프라인 학습과 온라인 서빙이 **동일한 피처 계산 로직**을 공유하도록(train/serve parity) 설계했고, 주기적 재학습 → 자동 검증 → 승격 → **서비스 반영까지** 사람 손 없이 이어지도록 파이프라인을 닫았습니다.
...
Airflow DAG(orchestration/dags/retrain.py) — 매주 월요일 05:00, build → retrain → promote
   메타데이터 DB: Postgres (fds-postgres)
```

핵심 설계 원칙 (`core/schema.py`, `core/store.py`, `serving/app.py` 주석 기반):
...
- **"어떻게 계산하는가"(`core/features.py`)와 "어디에 저장하는가"(`core/store.py`)를 분리**해서, 오프라인 빌드는 `InMemoryStore`(~61,000 events/sec), 실시간 스트림은 `RedisStore`(~2,500 events/sec, 컨슈머 재시작에도 이력 유지)를 같은 인터페이스로 교체해 쓸 수 있게 했다.
...
- **모델 교체는 서비스 재시작 없이 반영**된다. `promote.py`는 임시 파일에 쓴 뒤 이름을 바꿔치는 방식(원자적 rename)으로 `model.txt`를 교체하고, `serving/app.py`는 요청마다 파일 수정시각만 확인해 바뀌었을 때만 다시 읽는다. 덮어쓰기가 아니라 rename이어야 하는 이유는, 읽는 쪽이 반쯤 쓰인 파일을 볼 가능성을 없애기 위해서다.
...
| 6 | Column screening / Gain importance / SHAP / Permutation importance / Null importance로 검증 후 스키마 확장 (피처 15개, **AUC 0.7513 / PR-AUC 0.1837**) |

### Phase 6에서 실제로 한 것

원본 CSV에서 스키마가 처음부터 버렸던 컬럼 전부(415개)를 다시 꺼내, 단계별로 기여도를 측정했습니다.
...
결론: `id_31`, `id_19`, `id_20`, `id_29`, `id_30` 5개를 스키마에 추가 → **AUC 0.7235 → 0.7513, PR-AUC 0.1140 → 0.1837**.

Null importance에서 기존 피처 중 `amt_sum_1h`(0.21), `txn_count_1h`(0.32), `amt_sum_24h`(0.41)는 노이즈 기준선 바로 위에 걸쳐 있는 것으로 나왔습니다. 지금은 남겨뒀지만 "기여가 확실한 피처"는 아니라는 걸 알고 있는 상태입니다.

### Phase 6 이후 마무리

- **재학습 루프를 닫음** — 승격된 모델이 재시작 없이 서비스에 반영되도록 `serving/app.py`에 mtime 기반 재로드를, `offline/promote.py`에 원자적 파일 교체를 추가. 실서버에서 왕복 검증했다(아래 "검증 방법" 참고).
- **Airflow 메타데이터 DB를 SQLite → Postgres로 이전** — `docker-compose.yml`에 정의만 되고 아무도 안 쓰던 Postgres에 실제 역할을 줬다. 코드 변경 0줄, `airflow.cfg` 설정 세 줄.

## 겪었던 오류와 해결 과정

작업하면서 실제로 막혔던 부분과 원인, 조치를 남겨둡니다.

### 1. 카드별 최신 거래 캐시가 엉뚱한 답을 준 문제 (Phase 3)
`feat:<card1>`에 미리 계산해둔 피처를 캐시로 스코어링에 쓰려 했으나, 캐시된 피처는 "이 카드의 직전 거래" 기준이라 지금 채점 중인 거래와 어긋났다. 캐시를 제거하고 스코어링 요청마다 `core/features.py`로 즉시 재계산하도록 바꿨다.

### 2. `--workers 4`로 레이턴시가 오히려 급격히 나빠진 문제 (Phase 4)
동시성을 올리면 빨라질 거라 예상했지만 p50이 2.5ms → 44ms로 튀었다. 요청을 단계별로 나눠 측정해보니 서버 처리 시간(TTFB)은 그대로였고, 응답 헤더와 바디 사이에서 43ms가 사라졌다.
...
원인은 멀티프로세스 소켓 경로에 `TCP_NODELAY`가 빠져 있어, Nagle 알고리즘(작은 패킷을 모아 보냄)과 클라이언트의 지연 ACK(ACK를 모아 보냄)가 서로를 기다리다 40ms 타이머가 만료되는 상황이었다. "워커가 많아서 자원 경쟁"이 아니라 "소켓 설정이 빠져서"였다.
...
게이트를 시험하려고 일부러 약한 모델(`--warmup-days 150`)을 만들어 넣었는데, 오히려 정상 모델(0.7235)을 크게 이긴 0.7443이 나왔다. 원인은 `promote.py`가 후보를 채점할 때 **후보가 실제로 학습한 구간이 아니라 스크립트가 새로 만든 표준 분할**을 시험지로 썼기 때문이다. 약한 모델의 학습 구간(day 150~182)이 표준 시험지(day 140~182) 안에 통째로 들어가 있었다. 즉 0.7443은 실력이 아니라 암기였다.

"train/test 겹침 없음" 게이트를 추가하자 **시험지 118,108건 중 99,622건(84%)을 후보가 이미 학습했다**고 정확히 잡아냈다. 이 검사는 통계 판정이 아니라 이분법(겹치거나 아니거나)이라 임계값을 정할 필요도, 좋은 모델을 잘못 기각할 위험도 없다.
...
`feature_contribution.py` 1차 실행은 V/C/D/M 컬럼까지 전부 후보로 넣었고, AUC가 0.7235 → **0.8806**까지 올랐다. 그런데 이 컬럼들은 Vesta가 사후에 가공해둔 값이라 결제 단말이 실시간으로 보낼 수 있는 데이터가 아니다. 이 프로젝트가 처음부터 세운 설계 원칙을 분석 단계에서 스스로 어긴 셈이었다.
...
- **2차**: permutation importance 진입 직후 죽음 — sklearn의 `permutation_importance`가 `.fit` 메서드를 요구하는데 raw LightGBM `Booster` 객체에는 없어서 거부됨. 작은 래퍼 클래스로 감싸 해결.

두 번 다 마지막 단계까지 완주한 적이 없어서, 세 번째 실행이 17분 넘게 걸릴 때 "멈춘 것"과 "원래 오래 걸리는 것"을 구분하지 못했다. `n_repeats=10 × 후보 128개 = 1,280번`의 재예측이 정상 소요 시간이었다.
...
### 10. 그 통합에서 호출부 하나를 놓쳐 `/score`가 3주간 죽어 있던 문제
...
무서운 건 **아무도 몰랐다**는 점이다. parity 테스트 3개는 계속 통과했고(그건 피처 *계산*까지만 검사한다), 스키마를 확장한 뒤로 서버를 한 번도 안 켜봤기 때문이다. 위 "재학습 루프 닫기"를 검증하려고 uvicorn을 띄운 순간 첫 요청에서 드러났다.

교훈 두 가지: **공유 헬퍼로 통합할 때는 호출부를 전부 grep해야 하고**, **단위 테스트 통과는 서비스가 뜬다는 뜻이 아니다.**
...
포트폴리오 범위에서 의도적으로 남겨둔 부분입니다. 숨기는 것보다 적어두는 쪽이 맞다고 판단했습니다.
...
bench/        레이턴시 벤치마크
tests/        train/serve parity + 모델 재로드 테스트
```

## 검증 방법

숫자만 적어두면 믿을 이유가 없으니, 어떻게 확인했는지 남깁니다.
