...
이어서 재학습 루프를 닫았다. 승격된 모델이 재시작 없이 반영되도록 `serving/app.py`에 mtime 기반 리로드를, `offline/promote.py`에 원자적 파일 교체를 넣고 실서버에서 왕복 검증했다(아래 "검증 방법" 참고). 정의만 되고 쓰이지 않던 Postgres에는 Airflow 메타데이터 DB 역할을 주어 SQLite에서 이전했다.

## 겪었던 오류와 해결 과정

작업하면서 실제로 막혔던 것들. 원인과 조치를 적어둔다.
