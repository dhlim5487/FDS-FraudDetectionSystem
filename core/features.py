...
class FeatureEngine:
    """
    Holds per-card history and turns each incoming event into a feature dict.

    Create ONE engine and feed it events in event-time order. It keeps a small
    rolling history per card (max 7 days) plus the set of devices ever seen on
    that card, and recomputes the clues exactly from that history each time.
    """

    def __init__(self) -> None:
        # card1 -> deque of (dt, amount, addr1) for the last 7 days
    ...
    def process(self, event: TransactionEvent) -> dict[str, float]:
        ...
        card = event.card1
        hist = self._history[card]
        seen_devices = self._devices[card]
        t = event.TransactionDT
        amt = event.TransactionAmt

        # Drop anything older than our largest window (7 days) to bound memory.
        while hist and hist[0][0] < t - WEEK:
            hist.popleft()

        # Slice the history into the windows we care about. hist is already
        # trimmed to 7 days, so it IS the 7-day window.
        in_1h = [row for row in hist if row[0] >= t - HOUR]
        in_24h = [row for row in hist if row[0] >= t - DAY]

        amts_1h = [row[1] for row in in_1h]
        amts_24h = [row[1] for row in in_24h]
        amts_7d = [row[1] for row in hist]
        ...
        secs_since_prev = (t - hist[-1][0]) if hist else -1
        ...
        is_new_device = 1 if (device is not None and device not in seen_devices) else 0
